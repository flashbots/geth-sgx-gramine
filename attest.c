...
int main(int argc, char** argv) {
    ...
    if (argc > 2 && ra_tls_verify_lib) {
        if (argc != 6) {
            mbedtls_printf("USAGE: [APPLICATION_PORT=... APPLICATION_HOST=...] %s %s <expected mrenclave>"
                           " <expected mrsigner> <expected isv_prod_id> <expected isv_svn>\n"
